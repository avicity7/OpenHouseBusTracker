<script lang="ts">
  ...
  const updateBusVisibility = async(bus_id: string, visibility: boolean) => {
    await fetch(`${backend_uri}:3000/bus/update-bus-visibility/${bus_id}/${visibility}`, {
      method: 'PUT'
    })
    getBuses()
  }
  ...
  const deleteBus = async(bus_id: string) => {
    await fetch(`${backend_uri}:3000/bus/delete-bus/${bus_id}`, {
      method: 'DELETE'
    })
    getBuses()
  }
