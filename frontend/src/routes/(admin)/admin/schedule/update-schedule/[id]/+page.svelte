<script lang="ts">
	...
	if (StartTime.endsWith('Z')) {
		...
		selectedStartTime = `${d.getFullYear()}-${d.getMonth() < 10 ? "0" + d.getMonth() : d.getMonth()}-${d.getUTCDate() < 10 ? "0" + d.getUTCDate() : d.getUTCDate()}T${d.getHours() < 10 ? "0" + d.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second: '2-digit'}).split(' ')[0] : d.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second: '2-digit'}).split(' ')[0]}`
	} else {
		selectedStartTime = StartTime.split('+')[0]
	}
	if (EndTime.endsWith('Z')) {
		let d = new Date(EndTime)
		selectedEndTime = `${d.getFullYear()}-${d.getMonth() < 10 ? "0" + d.getMonth() : d.getMonth()}-${d.getUTCDate() < 10 ? "0" + d.getUTCDate() : d.getUTCDate()}T${d.getHours() < 10 ? "0" + d.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second: '2-digit'}).split(' ')[0] : d.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second: '2-digit'}).split(' ')[0]}`
	} else {
