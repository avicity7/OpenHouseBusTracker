<script lang="ts">
	...
	const formatTimestamp = (timestamp: string) => {
		...
		let formattedDate = utcDate.toLocaleString([], {year: 'numeric', day:'2-digit', month:'short',hour:'2-digit', minute: '2-digit'});
	...
	function filterSchedules() {
		filteredSchedules = busSchedule.filter((schedule) => {
			...
			const routeMatch =
				!selectedRoute || schedule.RouteName.toLowerCase() === selectedRoute.toLowerCase();
			const carplateMatch =
				!selectedCarplate || schedule.Carplate.toLowerCase() === selectedCarplate.toLowerCase();
			const startTimeMatch =
				!startTime ||
				formatTimestamp(schedule.StartTime).toLowerCase().includes(startTime.toLowerCase());
			const endTimeMatch =
				!endTime || formatTimestamp(schedule.EndTime).toLowerCase().includes(endTime.toLowerCase());
...
<div class="p-6 md:p-12">
	<div class="flex items-center justify-between mb-4">
		...
		<div class="ml-auto">
			<select
				class="border border-gray-300 text-sm rounded-xl px-3 py-2"
				bind:value={selectedRoute}
				on:change={filterSchedules}
			...
			<input
				...
				on:input={filterSchedules}
				data-testid="search-start-time"
			/>
			<input
				type="datetime"
					...
					{#each filteredSchedules as schedule (schedule.BusScheduleId)}
						<tr class="hover:bg-gray-100 schedule-item" data-testid="search-results">
							<td class="px-6 py-4 whitespace-nowrap">
								<input
									type="checkbox"
									checked={selectedSchedules.has(schedule.BusScheduleId)}
									on:change={() => {
							...
							<td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
								<div class="flex items-center">
									<a
										href={`schedule/update-schedule/${schedule.BusScheduleId}`}
									...
									<button
										...
										on:click={() => deleteSchedule(schedule.BusScheduleId)}
				...
				{:else if searchTerm !== '' || selectedCarplate !== '' || selectedRoute !== '' || startTime !== '' || endTime !== ''}
					<tr>
						<td colspan="8" class="px-6 py-4 whitespace-nowrap text-center text-gray-400"
							>No matching schedules found</td
						>
					</tr>
				{:else}
