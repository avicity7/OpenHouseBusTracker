<script lang="ts">
  export let data
  const { routes, backend_uri } = data

 const deleteRoute = async ( selectedRoute: string ) => {
  await fetch(`${backend_uri}:3000/admin/route-name/${routes}`, {
      method: 'DELETE'
    })
 }
...
<div class="p-6 md:p-12">
  ...
  <a href="/admin/bus-routes/create-route" class="bg-red-700 hover:bg-red-800 px-8 py-2 m-12 text-white rounded-md">
    Add Routes
  </a>
