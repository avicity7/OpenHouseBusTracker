...
async function updateDriver(event: Event) {

      const response = await fetch(`${PUBLIC_BACKEND_URL}:3000/driver/update-driver/${driver.DriverId}  `, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        console.log(`Driver has been updated to ${DriverName}!`)
        goto('/admin/drivers');
      } else {
        console.log(`Failed to update ${DriverName}`)
      }
    }
