...
export const actions = {
  signUp: async ({ request, cookies, locals }) => {
    ...
    try {
      const parsed = await result.json() as { User: { Name: string, Email: string, Role: string, VerificationToken: string, Contact: string }, AccessToken: string, RefreshToken: string }
